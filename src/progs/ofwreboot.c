...
int main(int argc, char**argv)
{
    ...
    openlog("openfirewallreboot", LOG_PID, LOG_USER);
    if (message[0]) {
        syslog(LOG_NOTICE, "%s", message);
    }
    if (flag_down) {
        syslog(LOG_NOTICE, "Ofw will shutdown and halt/power off");
        verbose_printf(1, "Ofw will shutdown and halt/power off ... \n");
        snprintf(command, STRING_SIZE_LARGE, "/sbin/shutdown -t 10 -h now %s", message);
        safe_system(command);
        return 0;
    }
    if (flag_boot) {
        syslog(LOG_NOTICE, "Ofw will shutdown and reboot");
        verbose_printf(1, "Ofw will shutdown and reboot ... \n");
        snprintf(command, STRING_SIZE_LARGE, "/sbin/shutdown -t 10 -r now %s", message);
        safe_system(command);
        return 0;
    }
    if (flag_bootfs) {
        syslog(LOG_NOTICE, "Ofw will shutdown and reboot with fsck forced");
        verbose_printf(1, "Ofw will shutdown and reboot with fsck forced ... \n");
