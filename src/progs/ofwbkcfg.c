 ...
 *
 * $Id: openfirewallbkcfg.c 5130 2010-11-15 16:49:55Z owes $
 *
 */
...
int cmb_files(char *outfilename, const char *outfname, const char *fnamebase, const int verbose)
{
    ...
    sprintf(dirname, "/var/ofw/backup");
        ...
        while (namecount--) {
            ...
            if (strncasecmp(fnamebase, namelist[namecount]->d_name, strlen(fnamebase)) == 0) {
                ...
                if (strncmp(namelist[namecount]->d_name, EXCLUDE_HARDWARE, strlen(EXCLUDE_HARDWARE))) {
                    ...
                    sprintf(addfilename, "/var/ofw/backup/%s", namelist[namecount]->d_name);
                    if (verbose)
...
void writetagfile(const char *filename, const char *timestamp, const char *comment)
{
    if (!(tagfile = fopen(filename, "w"))) {
        perror("openfirewallbkcfg : Unable to open tagfile\n");
        exit(BACKUP_ERR_ANY);
    }
...
void exithandler(void)
{
    ...
    snprintf(command, STRING_SIZE - 1, "/var/ofw/backup.dat.time");
    unlink(command);
}
...
static void usage()
{
    ...
    fprintf(stderr, "openfirewallbkcfg --keycat backup_password " TMPLOGFILE "\n");
    fprintf(stderr, "\tDisplay the key crypted with backup_password\n");
    fprintf(stderr, "\tif backup_password match\n");
    fprintf(stderr, "openfirewallbkcfg --keyexist\n");
    fprintf(stderr, "\tTest if backup.key file exist\n");
    fprintf(stderr, "openfirewallbkcfg --mount sda1\n");
    fprintf(stderr, "\tMount /dev/sda1 device\n");
    fprintf(stderr, "openfirewallbkcfg --umount\n");
    fprintf(stderr, "\tUmount removable device mounted on " MOUNTPOINT "\n");
    fprintf(stderr, "openfirewallbkcfg --write 'comment' [--verbose]\n");
...
int main(int argc, char *argv[])
{
    ...
    if (system("echo `grep -m 1 '::version' /usr/lib/ofw/general-functions.pl | cut -f 2 -d \"'\"` > /var/ofw/backup/version")) {
