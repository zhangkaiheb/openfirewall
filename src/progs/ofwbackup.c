 ...
 *
 * $Id: ofwbackup.c 4410 2010-03-26 07:58:35Z owes $
 *
 */
...
int testfloppywrite(char *device)
{
    ...
    if (S_ISBLK(st.st_mode)) {
        ...
        /* dd does not work inside ofwbackup detecting an absent
...
int main(void)
{
    ...
    if (!(systeminclude = open("/var/ofw/backup/include.system", O_RDONLY))) {
    ...
    if (!(userinclude = open("/var/ofw/backup/include.user", O_RDONLY))) {
    ...
    snprintf(command, STRING_SIZE - 1,
             "/bin/tar -T %s"
             " -X /var/ofw/backup/exclude.system"
             " -X /var/ofw/backup/exclude.user" " -C / -czf %s &>/dev/null", tempincfilename, temptestfilename);
    ...
    snprintf(command, STRING_SIZE - 1,
             "/bin/tar -T %s"
             " -X /var/ofw/backup/exclude.system"
             " -X /var/ofw/backup/exclude.user" " -C / -czf %s &>/dev/null", tempincfilename, floppydev);
    safe_system(command);
