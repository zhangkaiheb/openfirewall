...
#
# Copyright (c) 2009-2012 The Openfirewall Team
#
...
{
    @SCHEDULER::list = ();
    if (! -e '/var/ofw/main/scheduler') {
        return;
    }

    open(FILE, '/var/ofw/main/scheduler');
    my @tmpfile = <FILE>;
    close (FILE);
...
sub writeSettings
{
    open(FILE, '>/var/ofw/main/scheduler') or die 'Unable to open scheduler settings file.';
    ...
    system('/usr/bin/sort -t "," -k 3 -o /var/ofw/main/scheduler /var/ofw/main/scheduler');
