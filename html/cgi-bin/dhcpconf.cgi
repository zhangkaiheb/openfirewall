...
# Add entry in menu
# MENUTHRDLVL "dhcp server" 010 "dhcp configuration" "dhcp configuration"
...
if ($dhcpsettings{'ACTION'} eq $Lang::tr{'edit'} || $dhcpsettings{'ACTION'} eq $Lang::tr{'add interface'}) {
...
<tr align='center'>
    <td><strong>$dhcpsettings{'ACTION'}</strong></td>
</tr>
</table>
END
...
<tr>
    ...
    <td width='2%'></td>
    <td width='25%' class='base'>$Lang::tr{'start address'}-$Lang::tr{'end address'}<img src='$blob' alt='*' /></td>
    <td width='50%'><input type='text' name='START_ADDR_${ic}' value='$dhcpsettings{"START_ADDR_${ic}"}' $disable_text />&nbsp;-&nbsp;<input type='text' name='END_ADDR_${ic}' value='$dhcpsettings{"END_ADDR_${ic}"}' $disable_text />
    </td>
</tr>
END
        ;
...
END
    if ($dhcpsettings{'ACTION'} eq $Lang::tr{'edit'}) {
        ...
        print "<input type='hidden' class='commonbuttons' name='ACTION' value='SAVE_MAIN' />\n";
    }
    print <<END;
        &nbsp&nbsp
    ...
    <td  class='onlinehelp'>
        <a href='${General::adminmanualurl}/services-dhcp.html' target='_blank'><img src='/images/web-support.png' alt='$Lang::tr{'online help en'}' title='$Lang::tr{'online help en'}' /></a>
</td>
</table>
</form>
END
